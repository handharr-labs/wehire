...
const INPUT_CLASS =
  'w-full border border-stroke rounded-md px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)]';
  ...
  return (
    <main className="min-h-screen bg-page">
      <div className="max-w-2xl mx-auto px-4 py-10">
        <h1 className="text-2xl font-bold text-ink mb-2">Apply for {job.title}</h1>
        <p className="text-sm text-ink-muted mb-8">{company.name}</p>

        <form onSubmit={onSubmit} noValidate className="bg-surface rounded-lg border border-stroke p-8 space-y-5">
...
function FormFieldInput({ field, fieldErrors, onCvFileChange }: FieldInputProps) {
  ...
  if (field.fieldName === 'cv_url') {
    return (
      <div>
        <label className="block text-sm font-medium text-ink mb-1" htmlFor="cvFile">
  ...
  if (field.type === 'textarea') {
    return (
      <div>
        <label className="block text-sm font-medium text-ink mb-1" htmlFor={inputName}>
